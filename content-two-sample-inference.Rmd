...
df <- tibble('Sample 1'=c(sample1, paste('Mean:',mean(sample1))), 
             'Sample 2'=c(sample2, paste('Mean:',mean(sample2)))) %>% t()
kbl(df) %>% 
...
Now we'll create as many permutations as we can, reassigning the "Sample 1" and "Sample 2" labels across all the observations. There are $\binom {m+n}{m} = \binom {8}{4} = 70$ possible permutations. We'll calculate a distribution of test statistics by finding what $D^*$ is for each permutation:
...
df <- tibble('S. 1*'=sample1.1, 'S. 2*'=sample2.1,
             'S. 1**'=sample1.2, 'S. 2**'=sample2.2,
             'etc'=sample.x, 
             'S. 1***'=sample1.3, 'S. 2***'=sample2.3,
             )
row.names(df) <- NULL
...
df2 <- tibble('D*'=d.1,'D**'=d.2,'etc'='...', 'D***'=d.3,)
row.names(df2) <- NULL
...
```{r,echo=F, eval=T, results=T}
perm.num <- c(1, fa('ellipsis-v', fill='black'), 68:70)
U.stat <- c(0, fa('ellipsis-v', fill='black'), 15, 15, 16)
...
The intuition here is fairly straightforward: We expect to see a test statistic $U$ as or more extreme than $2$ 5.7% of the time when we assume that there is no difference in the null hypothesis.
