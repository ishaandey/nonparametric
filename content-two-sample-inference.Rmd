...
Say you're expecting population 1 to be *lower* that population 2. You'll use a *lower-tail* test, and use a set of hypotheses like so: 
$$
H_0: \mu_1 = \mu_2 \\
H_a: \mu_1 \leq \mu_2
$$
...
$F_(x)$ here refers to the empirical CDF of the function, or *probability of seeing a value less than or below* $x$.
...
pdfs <- ggplot(data=df, aes(x=x)) + 
  geom_line(aes(y = pdf.1, color = "Mean = 0")) + 
  geom_line(aes(y = pdf.2, color = "Mean = 1"), linetype='dashed') +
   theme(axis.title.x = element_blank(),
        panel.grid.minor = element_blank(),
        plot.title = element_text(hjust = 0.5),
        legend.position = c(0.15,0.75)) +
  ylab('Probability Density') + ggtitle('Comparing Distributions') +
  scale_color_manual(values = c("Mean = 0" = "blue", "Mean = 1" = "red")) +
  labs(colour = "Location Parameter") +
  scale_y_continuous(breaks=c(0))

  
cdfs <- ggplot(data=df, aes(x=x)) + 
  geom_line(aes(y = cdf.1), color = "blue") + 
  geom_line(aes(y = cdf.2), color = "red", linetype='dashed') +
