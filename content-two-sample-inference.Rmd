...
From the first plot we see $\mu_1 < \mu_2$, and from the second, $F_1(x) \geq F_2(x)$.
...
The two-sample t-test is by far the most powerful test when the assumptions are met for comparing means between two samples.
...
```{r}
source('https://raw.githubusercontent.com/ishaandey/nonparametric/main/helper.R')

sample1 <- c(37, 49, 55, 57)
sample2 <- c(23, 31, 39, 46)
...
There's only one **assumption**, that both population distribution are continuous (not categorical / discrete)
...
kbl(df2[c('Values','Ranks')] %>% t(), 
    escape=F, align='c') 
```

$W_{obs}$ is simply the *sum of ranks* of sample 1 observations: $W_{obs}=$ <span style="color: #79759d;">1+2+3+5</span> $=11$.
...
<hr style="height:1px; width:60%;"></hr>
...
In our example, there are $\binom {8}{4}=70$ possible ways we could've obtained the four observations in sample 1 from a total of eight values. We could lookup the significance from a table, or read off the p-value from the test output
...
Because it's less than our confidence threshold $\alpha$ of $0.05$, we reject the null hypothesis that $F_1(x) = F_2(x)$, and conclude that the location of population 1 is lower than population 2.
...
We then look at every possible combination of the two samples and compare the values. We check if the values of the first sample (the columns) are greater than the values of the second sample (rows):
...
kbl(df2, format='html', escape=F) %>%
  ...
  row_spec(0, bold=T, color=col2) %>%
  column_spec(1, bold=T, color = col1) %>%
  add_header_above(c("Is Sample 1 > Sample 2?" = 5))
```

Our test statistic $U_{obs}$ is the number of **Y**'s we found in the matrix, $=2$.
